...
function activate(context) {
  ...
  if (!workspaceRoot)
    return;
  if (!fs.existsSync(path.join(workspaceRoot, ".tabulakit")))
    return;
...
function readSiteConfig(workspaceRoot) {
  const defaults = {
    name: "TabulaKit",
  ...
  try {
    ...
    if (nameMatch)
      defaults.name = nameMatch[1];
    const descMatch = content.match(/description:\s*"([^"]+)"/);
    if (descMatch)
      defaults.description = descMatch[1];
    const colorMatch = content.match(/color:\s*"([^"]+)"/);
    if (colorMatch)
      defaults.theme.color = colorMatch[1];
    const tealMatch = content.match(/tealColor:\s*"([^"]+)"/);
    if (tealMatch)
      defaults.theme.tealColor = tealMatch[1];
  } catch {
  }
  return defaults;
}
function ensurePreviewPort(workspaceRoot) {
...
function readTabulaKitConfig(workspaceRoot) {
  try {
...
function writeTabulaKitConfig(workspaceRoot, config) {
  try {
    fs.writeFileSync(
...
function startPreviewServer(workspaceRoot, port) {
...
function showDashboard(context, workspaceRoot, config, port) {
  if (dashboardPanel) {
...
function detectProjectState(workspaceRoot, port) {
  const hosting = [];
  const siteUrls = [];
  if (fs.existsSync(path.join(workspaceRoot, ".github", "workflows", "deploy.yml"))) {
    hosting.push("GitHub Pages");
    try {
      const remote = (0, import_child_process.execSync)("git remote get-url origin", { cwd: workspaceRoot, encoding: "utf8" }).trim();
      const m = remote.match(/github\.com[:/]([^/]+)\/([^/.]+)/);
      if (m)
        siteUrls.push(`https://${m[1].toLowerCase()}.github.io/${m[2]}/`);
    } catch {
    }
  }
  if (fs.existsSync(path.join(workspaceRoot, "firebase.json"))) {
    ...
    try {
      ...
      if (m)
        siteUrls.push(`https://${m[1]}.web.app`);
    } catch {
    }
  }
...
function executePublish(workspaceRoot) {
  ...
  vscode.window.withProgress(
    ...
    async () => {
      try {
        try {
          (0, import_child_process.execSync)("git diff --quiet && git diff --cached --quiet", { cwd: workspaceRoot });
          const untracked = (0, import_child_process.execSync)("git ls-files --others --exclude-standard", { cwd: workspaceRoot, encoding: "utf8" }).trim();
          if (!untracked) {
        ...
        (0, import_child_process.execSync)("git add -A", { cwd: workspaceRoot });
        try {
          (0, import_child_process.execSync)("git diff --cached --quiet", { cwd: workspaceRoot });
          vscode.window.showInformationMessage("Nothing to publish \u2014 no changes found.");
          return;
        } catch {
        }
        (0, import_child_process.execSync)('git commit -m "Update site content"', { cwd: workspaceRoot });
        (0, import_child_process.execSync)("git push", { cwd: workspaceRoot, timeout: 3e4 });
        if (hasFirebase) {
          try {
            (0, import_child_process.execSync)("firebase deploy --only hosting", { cwd: workspaceRoot, timeout: 6e4 });
          } catch (e) {
...
async function checkForUpdates(workspaceRoot) {
  const UPSTREAM_REPO = "https://github.com/heatherstoneio/tabulakit.git";
  const FRAMEWORK_FILES = [
    "site/index.html",
    "site/css/tabulakit.css",
    ".github/workflows/deploy.yml",
    "firebase.json",
    "netlify.toml",
    ".claude/CLAUDE.md",
    ".claude/commands/publish.md",
    ".claude/commands/tabula-update.md",
    ".claude/settings.json"
  ];
  try {
    try {
      const remotes = (0, import_child_process.execSync)("git remote -v", { cwd: workspaceRoot, encoding: "utf8" });
      if (!remotes.includes("upstream")) {
        (0, import_child_process.execSync)(`git remote add upstream ${UPSTREAM_REPO}`, { cwd: workspaceRoot, encoding: "utf8" });
      }
    } catch {
      return;
    }
    try {
      (0, import_child_process.execSync)("git fetch upstream main --quiet", { cwd: workspaceRoot, encoding: "utf8", timeout: 15e3 });
    } catch {
      return;
    }
    ...
    let diff = "";
    try {
      diff = (0, import_child_process.execSync)(`git diff HEAD upstream/main --stat -- ${fileArgs}`, { cwd: workspaceRoot, encoding: "utf8" }).trim();
    } catch {
      return;
    }
    if (!diff) {
      ...
      try {
        config.lastUpstreamSync = (0, import_child_process.execSync)("git rev-parse upstream/main", { cwd: workspaceRoot, encoding: "utf8" }).trim();
        ...
        async () => {
          try {
            for (const f of FRAMEWORK_FILES) {
              try {
                (0, import_child_process.execSync)(`git checkout upstream/main -- "${f}"`, { cwd: workspaceRoot });
              } catch {
              }
            }
            (0, import_child_process.execSync)("git add -A", { cwd: workspaceRoot });
            try {
              ...
              (0, import_child_process.execSync)('git commit -m "chore: update TabulaKit framework to latest upstream"', { cwd: workspaceRoot });
              (0, import_child_process.execSync)("git push", { cwd: workspaceRoot, timeout: 3e4 });
            ...
            cfg.lastUpstreamSync = (0, import_child_process.execSync)("git rev-parse upstream/main", { cwd: workspaceRoot, encoding: "utf8" }).trim();
...
function getDashboardHtml(config, state, port) {
...
<html><head><meta name="color-scheme" content="dark"><style>
  ...
  <script>const vscode = acquireVsCodeApi();</script>
    ...
    view.webview.onDidReceiveMessage((msg) => {
      if (msg.command === "open")
        vscode.commands.executeCommand("tabulakit.showDashboard");
    });
  }
};
